...
function initFirebase(){
  if(!window.firebase){
    setSyncStatus("error","Cloud unavailable");
    return false;
  }
  if(!firebaseConfigReady()){
    setSyncStatus("error","Firebase setup required");
    return false;
  }
  try{
...
async function loadSharedStore(){
  ...
  db.settings={shopName:STORE_NAME};
...
async function startSingleShop(){
  ...
  try{
    // No visible login: Firebase creates/reuses an anonymous session automatically.
    if(!auth.currentUser){
      await auth.signInAnonymously();
    }
    await loadSharedStore();

    // Keep both phones reasonably fresh when returning to the app.
    document.addEventListener("visibilitychange",async()=>{
      if(document.visibilityState==="visible" && cloudReady){
        try{await loadSharedStore()}catch(e){console.error(e)}
      }
    });
  }catch(err){
    console.error(err);
    setSyncStatus("error","Cloud setup required");
  }
}
