...
let db=freshDB(), reportPeriod="today", formContext=null, currentUser=null, saveTimer=null, cloudReady=false;
let fb=null, auth=null, firestore=null;

function freshDB(shopName="MASTER SUPER STORE"){
  return {
    settings:{shopName:(shopName||"MASTER SUPER STORE").toUpperCase()},
    products:[],
    transactions:[],
    customers:[],
    suppliers:[]
...
function setSyncStatus(state,text){
  ...
  el.className="sync-status "+state;el.innerHTML=`<span></span> ${text}`;
}
...
function initFirebase(){
  if(!window.firebase){
    setAuthMessage("Firebase library could not load. Check internet and refresh.","error");
    return false;
  }
  if(!firebaseConfigReady()){
    setAuthMessage("Firebase setup is incomplete. Add your project config in firebase-config.js.","error");
    return false;
  }
  try{
    ...
    console.error(err);
    setAuthMessage("Firebase could not start. Check firebase-config.js.","error");
    return false;
  }
}
function queueCloudSave(){
  if(!cloudReady||!currentUser||!firestore)return;
...
async function saveCloudData(){
  if(!cloudReady||!currentUser||!firestore)return;
  try{
    await firestore.collection("stores").doc(currentUser.uid).set({
      data:db,
    ...
    console.error(err);
    setSyncStatus("error","Not Saved");
  }
}
function saveDB(){syncBrand();renderAll();queueCloudSave()}
...
function applyAccountIdentity(){
  // Single-shop branded edition: visible owner identity stays fixed.
}
function syncBrand(){
  document.title="MASTER SUPER STORE";
  $("#loginStoreName") && ($("#loginStoreName").textContent="MASTER SUPER STORE");
  $("#headerStoreName") && ($("#headerStoreName").textContent="MASTER SUPER STORE");
...
function switchAuthMode(mode){
  ...
  $("#loginForm")?.classList.toggle("hidden",!login);$("#registerForm")?.classList.toggle("hidden",!register);
  $("#loginTab")?.classList.toggle("active",login);$("#registerTab")?.classList.toggle("active",register);
  if($("#authSubheading"))$("#authSubheading").textContent=login
    ?"Sign in from any phone or network and open your own saved store."
    :"Create an account once. Your profile and store data will stay saved online.";
  setAuthMessage("");
}
...
async function profileForUser(user){
  ...
  const fallback={
    ownerName:user.displayName||"STORE USER",
    shopName:"MASTER SUPER STORE",
    email:user.email||"",
...
async function loadAccount(user){
  ...
  currentUser={
    ...
    ownerName:profile.ownerName||user.displayName||"STORE USER",
    shopName:profile.shopName||"MASTER SUPER STORE",
  ...
  if(snap.exists && snap.data()?.data){
    ...
    db=freshDB(currentUser.shopName);
    await storeRef.set({
      data:db,
      createdAt:firebase.firestore.FieldValue.serverTimestamp(),
      updatedAt:firebase.firestore.FieldValue.serverTimestamp()
  ...
  db.settings=db.settings||{shopName:currentUser.shopName};
  ...
  cloudReady=true;
  syncBrand();renderAll();setSyncStatus("synced","Saved");
}
async function showAppForUser(user){
  try{
    ...
    console.error(err);
    currentUser=null;cloudReady=false;db=freshDB();
    $("#appShell").classList.add("hidden");
    $("#loginScreen").classList.remove("hidden");
    setAuthMessage("Your store could not be loaded. Check internet, Firebase setup and Firestore rules.","error");
  }
}
function friendlyAuthError(err){
  ...
  if(c.includes("email-already-in-use"))return "An account with this email already exists. Please sign in.";
  if(c.includes("invalid-email"))return "Enter a valid email address.";
  if(c.includes("weak-password"))return "Use a stronger password with at least 8 characters.";
  if(c.includes("too-many-requests"))return "Too many attempts. Please try again later.";
  if(c.includes("network-request-failed"))return "Internet connection problem. Please try again.";
  return err?.message||"Something went wrong.";
}
async function loginUser(email,password){
  if(!auth){setAuthMessage("Firebase setup is incomplete.","error");return}
  ...
  try{
    ...
    await showAppForUser(cred.user);
  }catch(err){setAuthMessage(friendlyAuthError(err),"error")}
  finally{setAuthBusy("loginSubmit",false,"")}
}
async function registerUser(){
  if(!auth||!firestore){setAuthMessage("Firebase setup is incomplete.","error");return}
  const owner=$("#registerOwnerName")?.value.trim();
  const shop=($("#registerShopName")?.value.trim()||"MASTER SUPER STORE").toUpperCase();
  ...
  setAuthMessage("Creating your private store…","info");
  try{
    const cred=await auth.createUserWithEmailAndPassword(email,password);
    await cred.user.updateProfile({displayName:owner});

    await firestore.collection("profiles").doc(cred.user.uid).set({
      ownerName:owner,
      shopName:shop,
    ...
    db=freshDB(shop);
    await firestore.collection("stores").doc(cred.user.uid).set({
      data:db,
      createdAt:firebase.firestore.FieldValue.serverTimestamp(),
      updatedAt:firebase.firestore.FieldValue.serverTimestamp()
    });
  ...
  finally{setAuthBusy("registerSubmit",false,"")}
}
async function logoutUser(){
  try{if(cloudReady)await saveCloudData()}catch(e){}
  try{await auth?.signOut()}catch(e){}
  currentUser=null;cloudReady=false;db=freshDB();$("#modal")?.close();
  $("#appShell").classList.add("hidden");$("#loginScreen").classList.remove("hidden");
  if($("#loginPassword"))$("#loginPassword").value="";
  switchAuthMode("login");setAuthMessage("Signed out securely.","success");
}
function restoreSession(){
  if(!initFirebase())return;
  auth.onAuthStateChanged(async user=>{
    if(user){
      if(!currentUser || currentUser.uid!==user.uid)await showAppForUser(user);
    }else{
...
function openModal(kind,ctx={}){
  ...
  if(kind==="settings"){t.textContent="Store Settings";b.innerHTML=`<div class="form-grid">${f("Owner Name","ownerName","text",{required:true,value:currentUser?.ownerName||"",full:true})}${f("Shop Name","shopName","text",{required:true,value:db.settings.shopName,full:true})}<div class="form-group full"><div class="balance-info"><span>Account Email</span><strong>${esc(currentUser?.email||"")}</strong><small>Your profile and all store records are saved in the backend database.</small></div></div></div><button class="secondary-btn logout-modal-btn" id="logoutBtn" type="button">Log Out</button>`;setTimeout(()=>{$("#logoutBtn")?.addEventListener("click",logoutUser)},0)}
  $("#modal").showModal()
}
...
function saveForm(fd){
  ...
  try{
    ...
    if(kind==="product"){
      ...
      const owner=fd.get("ownerName")?.trim(),name=fd.get("shopName")?.trim();
      if(!owner)throw Error("Owner name is required.");
      if(!name)throw Error("Shop name is required.");
      db.settings.shopName=name.toUpperCase();
      currentUser.ownerName=owner;
      currentUser.shopName=db.settings.shopName;
      firestore.collection("profiles").doc(currentUser.uid).set({
        ownerName:owner,
      ...
      .then(()=>syncBrand())
      .catch(err=>{console.error(err);setSyncStatus("error","Profile Error")});
    }
