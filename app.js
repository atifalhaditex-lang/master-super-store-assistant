...
let db=freshDB(), reportPeriod="today", formContext=null, currentUser=null, saveTimer=null, backendReady=false;
db.products.forEach(p=>{if(p.active===undefined)p.active=true});

function freshDB(shopName="MASTER SUPER STORE"){
  const x=structuredClone(seed);
  x.settings.shopName=(shopName||"MASTER SUPER STORE").toUpperCase();
...
async function api(path,options={}){
  ...
  if(!res.ok)throw Error(payload.error||`Request failed (${res.status})`);
  return payload;
}
function queueBackendSave(){
  ...
  saveTimer=setTimeout(saveBackendData,400);
}
async function saveBackendData(){
  if(!backendReady||!currentUser)return;
  try{
    await api("/api/store",{method:"PUT",body:JSON.stringify({data:db})});
    setSyncStatus("synced","Saved");
  }catch(err){
    console.error(err);setSyncStatus("error","Not Saved");
  }
}
function saveDB(){syncBrand();renderAll();queueBackendSave()}
...
function applyAccountIdentity(){
  const name=currentUser?.ownerName||"STORE USER", email=currentUser?.email||"";
  $("#accountOwnerName") && ($("#accountOwnerName").textContent=name.toUpperCase());
  $("#accountEmail") && ($("#accountEmail").textContent=email);
  $("#accountInitials") && ($("#accountInitials").textContent=initials(name));
}
function syncBrand(){
  document.title=db.settings.shopName;
  $("#loginStoreName") && ($("#loginStoreName").textContent=db.settings.shopName);
  $("#headerStoreName") && ($("#headerStoreName").textContent=db.settings.shopName);
...
function switchAuthMode(mode){
  ...
  if($("#authSubheading"))$("#authSubheading").textContent=login
    ?"Sign in to open your own saved store, profile and records."
    :"Create an account once. Your profile and store data will be saved in the backend database.";
  setAuthMessage("");
}
...
async function loadAccount(){
  ...
  db=result.data&&Object.keys(result.data).length?result.data:freshDB(result.user.shopName);
  db.settings=db.settings||{shopName:result.user.shopName||"MASTER SUPER STORE"};
...
async function showAppForUser(user){
  ...
  try{
    ...
    console.error(err);currentUser=null;backendReady=false;db=freshDB();
    $("#appShell").classList.add("hidden");$("#loginScreen").classList.remove("hidden");
    setAuthMessage("Your account data could not be loaded. Check the backend/server and try again.","error");
  }
}
async function loginUser(email,password){
  if(!email||!password){setAuthMessage("Enter email and password.","error");return}
  setAuthBusy("loginSubmit",true,"Signing in…");setAuthMessage("Checking your account…","info");
  try{
    const r=await api("/api/auth/login",{method:"POST",body:JSON.stringify({email,password})});
    setAuthMessage("");await showAppForUser(r.user);
...
async function registerUser(){
  ...
  setAuthBusy("registerSubmit",true,"Creating account…");setAuthMessage("Saving your account in the database…","info");
  try{
    const r=await api("/api/auth/register",{method:"POST",body:JSON.stringify({ownerName:owner,shopName:shop,email,password})});
    db=freshDB(shop);
    await showAppForUser(r.user);
  }catch(err){setAuthMessage(err.message,"error")}
  finally{setAuthBusy("registerSubmit",false,"")}
}
async function logoutUser(){
  try{if(backendReady)await saveBackendData()}catch(e){}
  try{await api("/api/auth/logout",{method:"POST",body:"{}"})}catch(e){}
  currentUser=null;backendReady=false;db=freshDB();$("#modal")?.close();
  $("#appShell").classList.add("hidden");$("#loginScreen").classList.remove("hidden");
...
async function restoreSession(){
  try{
    ...
    if(r.user)await showAppForUser(r.user);
  }catch(err){
    // 401 simply means no active login.
  }
}
...
function saveForm(fd){
  ...
  try{
    ...
    if(kind==="product"){
      ...
      api("/api/profile",{method:"PUT",body:JSON.stringify({ownerName:owner,shopName:db.settings.shopName})})
        .then(r=>{currentUser=r.user;syncBrand()})
