...
const LEGACY_DB_KEY="master_super_store_assistant_v1";
const userCacheKey=id=>`master_super_store_assistant_user_${id}`;
...
const supa=window.supabase?.createClient?window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY):null;
...
function loadDB(){
  // Never preload another account's store data before authentication.
  return freshDB();
}
function saveLocalUserCache(){
  if(currentUser) localStorage.setItem(userCacheKey(currentUser.id),JSON.stringify(db));
}
function loadLocalUserCache(userId){
  try{
    const x=JSON.parse(localStorage.getItem(userCacheKey(userId)));
    if(x) return x;
  }catch(e){}
  return null;
}
...
function saveDB(){saveLocalUserCache();syncBrand();renderAll();queueCloudSave()}
...
async function loadCloudData(){
  ...
  if(data?.data && Object.keys(data.data).length){
    ...
    const chosenShop=(currentUser?.user_metadata?.shop_name||"MASTER SUPER STORE").trim();
    // Brand-new account always starts with its own clean store.
    db=freshDB(chosenShop);
    const {error:upsertError}=await supa.from("store_data").upsert({
      user_id:currentUser.id,
      data:db,
      updated_at:new Date().toISOString()
    },{onConflict:"user_id"});
...
async function showAppForUser(user){
  ...
  try{
    ...
    setAuthMessage("Cloud data could not be loaded. For safety the store was not opened. Check internet and try again.","error");
  }
}
...
async function registerUser(){
  ...
  try{
    ...
    if(error)throw error;
    if(result.session&&result.user){db=freshDB(shop);await showAppForUser(result.user);}
...
async function logoutUser(){
  await saveCloudData();await supa.auth.signOut();currentUser=null;cloudReady=false;db=freshDB();$("#modal")?.close();$("#appShell").classList.add("hidden");$("#loginScreen").classList.remove("hidden");if($("#loginPassword"))$("#loginPassword").value="";switchAuthMode("login");setAuthMessage("Signed out securely.","success");
}
async function restoreSession(){
  if(!supa){setAuthMessage("Secure login service could not load. Check internet and refresh.","error");return}
...
function saveForm(fd){
  ...
  try{
    ...
    if(kind==="product"){
      ...
      const name=fd.get("shopName")?.trim();if(!name)throw Error("Shop name is required.");
