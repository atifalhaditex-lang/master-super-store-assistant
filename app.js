...
async function loadCloudData(){
  ...
  if(data?.data && Object.keys(data.data).length){
    ...
    // First cloud login: safely migrate current browser data into this account.
...
async function createAccount(){
  ...
  if(!email||!password){$("#loginMessage").textContent="Enter email and password first.";return}
  if(password.length<6){$("#loginMessage").textContent="Password must be at least 6 characters.";return}
  $("#loginMessage").textContent="Creating secure account...";
  ...
  if(data.session){
    ...
    $("#loginMessage").textContent="Account created. Please confirm your email, then log in.";
  }
}
async function logoutUser(){
  ...
  $("#loginPassword").value="";
  $("#loginMessage").textContent="Logged out securely.";
}
async function restoreSession(){
  const {data}=await supa.auth.getSession();
  if(data.session?.user) await showAppForUser(data.session.user);
}
